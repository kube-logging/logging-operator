...
GO_TEST_COVERAGE_VERSION := 2.19.0
...
test-e2e-nodeps:
	cd e2e && \
		...
		go test -count=1 -v -p ${E2E_CLUSTERS} -parallel ${E2E_SUITE_PARALLEL} -timeout ${E2E_TEST_TIMEOUT} $$(go list ./${E2E_TEST}/... | grep -vE '/e2e/internal(/|$$)')
