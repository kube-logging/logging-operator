...
func (n *nodeAgentInstance) configSecret() (runtime.Object, reconciler.DesiredState, error) {
	if n.nodeAgent.FluentbitSpec != nil {
		...
		input := fluentBitConfig{
			...
			Monitor:                 monitor,
			TargetHost:              fmt.Sprintf("%s.%s.svc%s", n.AggregatorQualifiedName(fluentd.ServiceName), n.logging.Spec.ControlNamespace, n.logging.ClusterDomainAsSuffix()),
...
func (n *nodeAgentInstance) generateUpstreamNode(index int32) upstreamNode {
	podName := n.AggregatorQualifiedName(fmt.Sprintf("%s-%d", fluentd.ComponentFluentd, index))
	return upstreamNode{
		Name: podName,
		Host: fmt.Sprintf("%s.%s.%s.svc%s",
			podName,
			n.AggregatorQualifiedName(fluentd.ServiceName+"-headless"),
