...
| check_apikey_on_start | bool | No | - | Check AWS key on start<br> |
| grant_read | string | No | - | Allows grantee to read the object data and its metadata<br> |
| overwrite | bool | No | - | Overwrite already existing path<br> |
...
| check_bucket | bool | No | - | Check bucket if exists or not<br> |
...
| compute_checksums | bool | No | - | AWS SDK uses MD5 for API request/response by default<br> |
...
| use_bundled_cert | bool | No | - | Use aws-sdk-ruby bundled cert<br> |
...
| force_path_style | bool | No | - | If true, the bucket name is always left in the request URI and never moved to the host as a sub-domain<br> |
| auto_create_bucket | bool | No | - | Create S3 bucket if it does not exists<br> |
| index_format | string | No | - | `sprintf` format for `%{index}`<br> |
| signature_version | string | No | - | Signature version for API Request (s3,v4)<br> |
| enable_transfer_acceleration | bool | No | - | If true, S3 Transfer Acceleration will be enabled for uploads. IMPORTANT: You must first enable this feature on your destination S3 bucket<br> |
| ssl_verify_peer | bool | No | - | If false, the certificate of endpoint will not be verified<br> |
| proxy_uri | string | No | - | URI of proxy environment<br> |
| grant_read_acp | string | No | - | Allows grantee to read the object ACL<br> |
| check_object | bool | No | - | Check object before creation<br> |
