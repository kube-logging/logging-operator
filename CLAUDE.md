...
- E2E tests use KIND: one directory per suite under `e2e/suites/`, each its own Go package and test binary, provisioning its own KIND cluster; `e2e/internal/harness` owns the lifecycle. Run a single suite with `make test-e2e E2E_TEST=<suite-dir>`. Knobs, conventions and how to add a suite: `e2e/README.md`
